...
MSortPartition::MSortPartition(unsigned const startIndex,
                               unsigned const endIndex)
    : start(startIndex), end(endIndex), size(endIndex - startIndex),
      index(startIndex) {}
...
MergeSort::MergeSort(std::vector<unsigned> &data)
    : Sort(data, Sort::Type::MERGE), state(STOP), partQ1(), partQ2(),
      readQ(partQ1), writeQ(partQ2) {}