...
bool HeapSort::IsHeapifying() const { return state == HEAPIFY_CHECK || state == HEAPIFY_INSERT; }
...
HeapSort::HeapSort(std::vector<unsigned> & data) : Sort(data, Sort::HEAP), state(STOP) { assert(data.size() > 1); }